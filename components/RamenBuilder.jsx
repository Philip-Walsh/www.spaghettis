...
export default function RamenBuilder() {
    ...
    useEffect(() => {
        if (steps.length === 0) {
            setSteps([...Object.values(menuOptions), { key: 'summary', label: 'Summary' }]);
        }
    }, [steps]);
    ...
    const filteredMenuOptions = veggieOnly
        ? Object.fromEntries(
              Object.entries(menuOptions).map(([key, step]) => [
                  ...
                  {
                      ...step,
                      choices: step.choices.filter((c) => veggieTags.includes(c.tag))
                  }
              ])
          )
        : menuOptions;
    ...
    const handleSelection = (stepKey, selectedItem) => {
        setSelectedItems((prev) => {
            ...
            if (Array.isArray(selectedItem)) {
                updated = {
                    ...prev,
                    [stepKey]: selectedItem.map((i) =>
                        typeof i === 'object' && i !== null && 'name' in i ? i.name : i
                    )
                };
            ...
            if (stepKey === 'noodleBase' && selectedItem.defaults) {
                Object.entries(selectedItem.defaults).forEach(([key, names]) => {
                    if (Array.isArray(names) && menuOptions[key]) {
                        updated[key] = names;
                    }
                });
            }
    ...
    return (
        <div className={styles['ramen-container']}>
            <div className={styles['page-content']} style={{ maxWidth: 'clamp(360px, 90%, 1200px)' }}>
                <h1 className={styles['title'] + ' animated-gradient'}>
                    {isSummaryStep ? 'Selection Complete!' : 'Build Your Perfect Ramen'}
                </h1>
                {isSummaryStep ? (
                    <SummaryView selectedItems={selectedItems} totalPrice={totalPrice} onOrderMore={handleOrderMore} />
                ) : (
